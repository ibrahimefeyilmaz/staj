...
class VerticalAltitudeBar(QWidget):
    ...
    def set_altitude(self, value):
        self.current_altitude = max(0.0, float(value))
        self.update()
    ...
    def paintEvent(self, event):
        ...
        for i in range(num_ticks + 1):
            tick_alt = i * 1000
            tick_y = bar_y + bar_height - ((tick_alt / self.max_altitude) * bar_height)
            ...
            painter.drawText(0, int(tick_y - 10), int(bar_x - 12), 20, Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter, f"{tick_alt}")

        # 4. ANLIK YÜKSEKLİK SİYAH ÇİZGİSİ
        ratio = min(1.0, self.current_altitude / self.max_altitude)
